window.TOURFORU_DATA = {
  destinations: [
    {
      id: "tongyeong-mireuksan",
      city: "통영시",
      district: "산양읍",
      title: "미륵산에서 만나는 한려수도",
      summary: "케이블카를 타고 바다와 섬을 한눈에 담는 통영 대표 여행지",
      distance: "42km",
      time: "55분",
      rating: 4.9,
      views: "18.6만",
      visitors: "1,842명",
      stay: "약 2시간",
      cost: "약 18,000원",
      image: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1000&q=85",
      images: [
        "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&w=1000&q=85"
      ],
      tags: ["바다전망", "케이블카", "가족여행"]
    },
    {
      id: "geoje-windyhill",
      city: "거제시",
      district: "남부면",
      title: "바람의 언덕, 초록과 바다 사이",
      summary: "탁 트인 해안과 풍차가 어우러진 거제의 대표 드라이브 코스",
      distance: "68km",
      time: "1시간 20분",
      rating: 4.8,
      views: "14.2만",
      visitors: "1,327명",
      stay: "약 1시간 30분",
      cost: "무료",
      image: "https://images.unsplash.com/photo-1470770841072-f978cf4d019e?auto=format&fit=crop&w=1000&q=85",
      images: [
        "https://images.unsplash.com/photo-1470770841072-f978cf4d019e?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?auto=format&fit=crop&w=1000&q=85"
      ],
      tags: ["드라이브", "인생사진", "연인여행"]
    },
    {
      id: "namhae-daraengi",
      city: "남해군",
      district: "남면",
      title: "다랭이마을 계단식 바다 풍경",
      summary: "산비탈 논과 푸른 바다가 층층이 이어지는 남해의 느린 여행지",
      distance: "91km",
      time: "1시간 45분",
      rating: 4.7,
      views: "11.8만",
      visitors: "946명",
      stay: "약 2시간",
      cost: "무료",
      image: "https://images.unsplash.com/photo-1500534623283-312aade485b7?auto=format&fit=crop&w=1000&q=85",
      images: [
        "https://images.unsplash.com/photo-1500534623283-312aade485b7?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1510798831971-661eb04b3739?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?auto=format&fit=crop&w=1000&q=85"
      ],
      tags: ["마을여행", "걷기", "바다"]
    },
    {
      id: "sancheong-valley",
      city: "산청군",
      district: "삼장면",
      title: "숲과 계곡이 만든 여름 피서지",
      summary: "맑은 물과 깊은 숲에서 쉬어가는 산청 자연 여행",
      distance: "79km",
      time: "1시간 25분",
      rating: 4.6,
      views: "8.7만",
      visitors: "725명",
      stay: "약 3시간",
      cost: "무료",
      image: "https://images.unsplash.com/photo-1432405972618-c60b0225b8f9?auto=format&fit=crop&w=1000&q=85",
      images: [
        "https://images.unsplash.com/photo-1432405972618-c60b0225b8f9?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1448375240586-882707db888b?auto=format&fit=crop&w=1000&q=85",
        "https://images.unsplash.com/photo-1428908728789-d2de25dbd4e2?auto=format&fit=crop&w=1000&q=85"
      ],
      tags: ["계곡", "숲", "가족피서"]
    }
  ],

  hotels: [
    {
      id: "tongyeong-oceanview-stay",
      name: "통영 오션뷰 스테이",
      type: "hotel",
      address: "경남 통영시 산양읍",
      description: "통영 앞바다를 바라보며 머물 수 있는 오션뷰 숙소입니다. 관광지 이동 동선이 짧고 가족 여행객의 만족도가 높은 곳입니다.",
      checkin: "15:00",
      checkout: "11:00",
      price: "128,000원",
      distance: "12분",
      rating: 4.8,
      reviews: "후기 824",
      image: "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=800&q=85"
    },
    {
      id: "geoje-marina-resort",
      name: "거제 마리나 리조트",
      type: "hotel",
      address: "경남 거제시 일운면",
      description: "마리나와 해안 산책로를 함께 이용할 수 있는 리조트입니다. 커플과 가족 단위 여행에 적합합니다.",
      checkin: "15:00",
      checkout: "11:00",
      price: "164,000원",
      distance: "18분",
      rating: 4.7,
      reviews: "후기 612",
      image: "https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?auto=format&fit=crop&w=800&q=85"
    },
    {
      id: "namhae-forest-pension",
      name: "남해 숲속 펜션",
      type: "hotel",
      address: "경남 남해군 남면",
      description: "숲과 바다 사이에서 조용하게 쉬어갈 수 있는 독채형 펜션입니다. 장기 체류와 느린 여행에 어울립니다.",
      checkin: "16:00",
      checkout: "11:00",
      price: "96,000원",
      distance: "22분",
      rating: 4.6,
      reviews: "후기 391",
      image: "https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?auto=format&fit=crop&w=800&q=85"
    }
  ],

  foods: [
    {
      id: "tongyeong-sea-table",
      name: "통영 바다 한상",
      type: "food",
      address: "경남 통영시 중앙동",
      description: "통영 제철 해산물과 지역 음식을 한 상으로 즐길 수 있는 현지식 식당입니다.",
      menu: "해산물 한상 · 충무김밥 · 생선구이",
      hours: "11:00~21:00",
      price: "25,000원",
      distance: "8분",
      rating: 4.8,
      reviews: "후기 1,204",
      image: "https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&w=800&q=85"
    },
    {
      id: "geoje-seafood-pot",
      name: "거제 해물뚝배기",
      type: "food",
      address: "경남 거제시 일운면",
      description: "싱싱한 해산물을 매콤하고 뜨끈한 뚝배기로 내는 거제 대표 현지 음식점입니다.",
      menu: "해물뚝배기 · 멍게비빔밥 · 생선구이",
      hours: "10:30~20:30",
      price: "18,000원",
      distance: "11분",
      rating: 4.7,
      reviews: "후기 916",
      image: "https://images.unsplash.com/photo-1569058242253-92a9c755a0ec?auto=format&fit=crop&w=800&q=85"
    },
    {
      id: "namhae-anchovy-ssambap",
      name: "남해 멸치쌈밥",
      type: "food",
      address: "경남 남해군 남면",
      description: "남해의 대표 식재료인 멸치를 조림과 쌈밥으로 즐기는 지역 향토 음식점입니다.",
      menu: "멸치쌈밥 · 멸치회무침 · 갈치구이",
      hours: "10:00~20:00",
      price: "16,000원",
      distance: "15분",
      rating: 4.6,
      reviews: "후기 742",
      image: "https://images.unsplash.com/photo-1515003197210-e0cd71810b5f?auto=format&fit=crop&w=800&q=85"
    }
  ],

  vehicles: [
    {
      id: "carnival-premium",
      badge: "광고",
      fit: "최적합",
      name: "카니발 9인승 프리미엄",
      start: "창원 성산구",
      arrival: "약 25분",
      price: "198,000원",
      uses: "482회",
      rating: 4.9,
      reviews: "후기 327",
      image: "https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=1000&q=85"
    },
    {
      id: "staria-lounge",
      badge: "",
      fit: "적합",
      name: "스타리아 라운지 11인승",
      start: "김해시",
      arrival: "약 38분",
      price: "215,000원",
      uses: "312회",
      rating: 4.8,
      reviews: "후기 241",
      image: "https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=1000&q=85"
    },
    {
      id: "solati-tour",
      badge: "추천",
      fit: "단체 적합",
      name: "쏠라티 투어 15인승",
      start: "창원 의창구",
      arrival: "약 32분",
      price: "278,000원",
      uses: "205회",
      rating: 4.7,
      reviews: "후기 164",
      image: "https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=1000&q=85"
    }
  ],

  guides: [
    {
      id: "guide-bluewave",
      nickname: "푸른물결",
      realName: "김민수",
      safePhone: "0504-1824-1001",
      region: "통영시",
      nearby: "미륵산에서 12분",
      careerYears: 6,
      tourCount: 428,
      rating: 4.9,
      educationCompleted: true,
      languages: ["한국어", "영어"],
      intro: "통영의 섬과 골목, 드라마 촬영지를 중심으로 이야기 여행을 안내합니다.",
      reviews: [
        "아이들과 함께 듣기 좋은 설명이었습니다.",
        "사진 촬영 지점을 세심하게 알려줬어요.",
        "관광객이 붐비지 않는 시간대를 잘 안내했습니다."
      ],
      image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=600&q=85"
    },
    {
      id: "guide-seagull",
      nickname: "통영갈매기",
      realName: "박정훈",
      safePhone: "0504-1824-1002",
      region: "통영시",
      nearby: "미륵산에서 18분",
      careerYears: 3,
      tourCount: 186,
      rating: 4.8,
      educationCompleted: true,
      languages: ["한국어", "일본어"],
      intro: "통영의 음식과 전통시장, 근대문화 이야기를 편안하게 소개합니다.",
      reviews: [
        "현지 음식점을 취향에 맞게 추천해줬어요.",
        "어르신과 함께 이동하기 편한 코스를 짜줬습니다."
      ],
      image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=600&q=85"
    },
    {
      id: "guide-greenwind",
      nickname: "초록바람",
      realName: "이서연",
      safePhone: "0504-1824-1003",
      region: "거제시",
      nearby: "바람의 언덕에서 9분",
      careerYears: 5,
      tourCount: 351,
      rating: 4.9,
      educationCompleted: true,
      languages: ["한국어", "영어", "중국어"],
      intro: "거제 자연여행과 가족 체험 코스를 전문으로 안내합니다.",
      reviews: [
        "아이들이 지루하지 않게 설명해줬습니다.",
        "관광지 이동 순서가 매우 효율적이었어요."
      ],
      image: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=600&q=85"
    },
    {
      id: "guide-slowtrip",
      nickname: "느린여행자",
      realName: "최현우",
      safePhone: "0504-1824-1004",
      region: "남해군",
      nearby: "다랭이마을에서 14분",
      careerYears: 2,
      tourCount: 94,
      rating: 4.7,
      educationCompleted: false,
      languages: ["한국어"],
      intro: "남해의 마을과 산책길을 천천히 걷는 소규모 여행을 안내합니다.",
      reviews: [
        "말수가 많지 않고 차분해서 좋았습니다.",
        "사진을 찍을 시간을 충분히 줬어요."
      ],
      image: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=600&q=85"
    }

  ],

  guideJobs: [
    {
      id: "job-tongyeong-0815",
      region: "통영시",
      areaType: "my",
      date: "2026-08-15",
      startTime: "09:00",
      endTime: "17:00",
      title: "미륵산·동피랑 하루 가이드",
      guests: "성인 4명",
      languages: ["한국어"],
      destinations: ["미륵산 케이블카", "통영 바다 한상", "동피랑 벽화마을"],
      meeting: "통영종합버스터미널",
      expectedPay: "180,000원",
      status: "모집중",
      guideNeeded: 1,
      description: "가족 여행객을 대상으로 통영 대표 관광지와 현지 음식을 안내하는 일정입니다."
    },
    {
      id: "job-geoje-0822",
      region: "거제시",
      areaType: "other",
      date: "2026-08-22",
      startTime: "10:00",
      endTime: "18:30",
      title: "거제 자연·촬영지 가이드",
      guests: "성인 2명 · 어린이 2명",
      languages: ["한국어", "영어 우대"],
      destinations: ["바람의 언덕", "신선대", "드라마 촬영지"],
      meeting: "거제 고현버스터미널",
      expectedPay: "210,000원",
      status: "모집중",
      guideNeeded: 1,
      description: "어린이 눈높이의 자연 해설이 가능한 가이드를 우대합니다."
    },
    {
      id: "job-namhae-0905",
      region: "남해군",
      areaType: "other",
      date: "2026-09-05",
      startTime: "09:30",
      endTime: "16:30",
      title: "남해 느린 마을여행",
      guests: "성인 6명",
      languages: ["한국어"],
      destinations: ["다랭이마을", "멸치쌈밥", "해안 산책길"],
      meeting: "남해공용터미널",
      expectedPay: "195,000원",
      status: "마감임박",
      guideNeeded: 1,
      description: "마을 이야기와 사진 촬영을 중심으로 천천히 이동하는 여행입니다."
    },
    {
      id: "job-sancheong-0912",
      region: "산청군",
      areaType: "other",
      date: "2026-09-12",
      startTime: "08:30",
      endTime: "17:30",
      title: "산청 숲·계곡 생태여행",
      guests: "성인 8명",
      languages: ["한국어"],
      destinations: ["산청 계곡", "숲길", "지역 체험장"],
      meeting: "산청시외버스터미널",
      expectedPay: "230,000원",
      status: "모집예정",
      guideNeeded: 2,
      description: "단체 여행객의 안전한 숲길 이동과 생태 해설이 필요한 일정입니다."
    }
  ],

  trainings: [
    {
      id: "training-guide-safety-01",
      audience: "guide",
      status: "접수중",
      title: "지역관광 가이드 안전·응급대응",
      category: "필수교육",
      date: "2026-08-18",
      time: "10:00~17:00",
      place: "경남관광기업지원센터 교육장",
      capacity: 30,
      remaining: 8,
      fee: "무료",
      hours: "6시간",
      certificate: true,
      description: "여행 중 사고 예방, 응급상황 대응, 여행자 보호와 안심번호 운영을 다룹니다.",
      curriculum: ["관광안전 기본수칙", "응급환자 초동대응", "여행자 분쟁 예방", "안심번호와 개인정보 보호"]
    },
    {
      id: "training-guide-story-01",
      audience: "guide",
      status: "접수예정",
      title: "경남 관광 스토리텔링 실무",
      category: "역량교육",
      date: "2026-09-03",
      time: "13:00~18:00",
      place: "창원컨벤션센터",
      capacity: 40,
      remaining: 40,
      fee: "20,000원",
      hours: "5시간",
      certificate: true,
      description: "지역 역사·문화·촬영지를 여행객의 관심에 맞게 설명하는 방법을 배웁니다.",
      curriculum: ["관광 해설 구성법", "연령별 설명 전략", "촬영지 이야기 만들기", "현장 발표 실습"]
    },
    {
      id: "training-guide-complete-01",
      audience: "guide",
      status: "접수완료",
      title: "외국인 여행객 기본 응대",
      category: "이수교육",
      date: "2026-07-15",
      time: "10:00~16:00",
      place: "온라인 교육",
      capacity: 100,
      remaining: 0,
      fee: "무료",
      hours: "5시간",
      certificate: true,
      completed: true,
      description: "외국인 여행객 안내와 문화 차이, 신분 확인 절차를 학습했습니다.",
      curriculum: ["다문화 이해", "기본 관광영어", "외국인 신원확인", "분쟁 예방"]
    },
    {
      id: "training-operator-law-01",
      audience: "operator",
      status: "접수중",
      title: "여객사업자 관광운송 법정 실무",
      category: "사업자교육",
      date: "2026-08-25",
      time: "10:00~17:00",
      place: "경남교통문화연수원",
      capacity: 50,
      remaining: 14,
      fee: "30,000원",
      hours: "6시간",
      certificate: true,
      description: "관광운송 계약, 배차, 운임표시, 사고 대응과 사업자 준수사항을 다룹니다.",
      curriculum: ["관광운송 계약 실무", "운임과 취소 규정", "차량·기사 관리", "사고와 보험 처리"]
    },
    {
      id: "training-operator-service-01",
      audience: "operator",
      status: "접수예정",
      title: "관광차량 고객응대와 서비스 품질",
      category: "서비스교육",
      date: "2026-09-11",
      time: "13:00~17:00",
      place: "김해 중소기업비즈니스센터",
      capacity: 45,
      remaining: 45,
      fee: "무료",
      hours: "4시간",
      certificate: true,
      description: "관광객 맞이, 짐 처리, 일정 변경과 후기 대응 방법을 교육합니다.",
      curriculum: ["관광객 응대", "짐과 승하차 지원", "일정 변경 대응", "평점·후기 관리"]
    },
    {
      id: "training-operator-complete-01",
      audience: "operator",
      status: "접수완료",
      title: "운수업체 개인정보 보호",
      category: "이수교육",
      date: "2026-06-22",
      time: "14:00~18:00",
      place: "온라인 교육",
      capacity: 100,
      remaining: 0,
      fee: "무료",
      hours: "4시간",
      certificate: true,
      completed: true,
      description: "회원·기사·외국인 여행객의 개인정보 처리와 안전한 보관 방법을 학습했습니다.",
      curriculum: ["개인정보 기본원칙", "신분증 자료 처리", "안심번호 운영", "유출사고 대응"]
    }
  ]

};
